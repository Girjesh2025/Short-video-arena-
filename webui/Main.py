...
<div style="text-align: center; margin-bottom: 20px;">
    <h2 style="background: linear-gradient(45deg, #FF3366, #FF9933); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 800; margin: 0;">MoneyPrinter 💸</h2>
    <p style="color: #64748B; font-size: 14px; margin-top: 5px;">World-Class Short Video Generator</p>
</div>
...
if selected_menu == "⚙️ " + tr("System Settings"):
      ...
      if render_subtitles:
        with st.container(border=True):
            ...
            if "selected_voice_lang" in locals() and selected_voice_lang == "Hindi (India) 🇮🇳":
                default_font = "Poppins-Bold.ttf"
            saved_font_name = config.ui.get("font_name", default_font)
            if "selected_voice_lang" in locals() and selected_voice_lang == "Hindi (India) 🇮🇳" and saved_font_name == "MicrosoftYaHeiBold.ttc":
                saved_font_name = "Poppins-Bold.ttf"
    ...
    if start_button:
        ...
        st.markdown("### 🎬 Video Generation Progress")
        ...
        while task_thread.is_alive():
            ...
            if task:
                ...
                if progress <= 5:
                    ...
                    desc = "Generating video script using LLM..."
                elif progress <= 20:
                    desc = "Generating search terms for video materials..."
                elif progress <= 30:
                    desc = "Generating voice audio (TTS)..."
                elif progress <= 40:
                    desc = "Generating and aligning subtitles..."
                elif progress <= 50:
                    desc = "Downloading high-quality video materials from Pexels..."
                elif progress < 100:
                    desc = "Combining video clips, syncing audio and rendering final MP4..."
                else:
        ...
        status_msg.success(tr("Video Generation Completed") + "! 🎉")
        ...
        for item in saved_videos:
            ...
            with st.container(border=True):
                ...
                with col1:
                    ...
                    st.write("**Script:**")
                    st.write(script if script else "No script details saved.")
